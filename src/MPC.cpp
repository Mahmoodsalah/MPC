...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // cost  stored in the first element of `fg`.
    // additions to the cost will added to `fg[0]`.
    fg[0] = 0;
    ...
    for (int i = 0; i < N - 2; i++) {
      // tuning the steering value for smoother transitions
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // element vector and there are 10 timesteps.

  // Set model variables
  // Position of the vehicle in the map coordinate
  double x = state[0];
  double y = state[1];
  ...
  double v = state[3];
  // CTE (the error between the center of the road and the vehicle's position)
  double cte = state[4];
  // Orientation error
  double epsi = state[5];
  ...
  // set to -25 and 25 degrees (in radians).
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332;
    vars_upperbound[i] = 0.436332;
  }

  // Acceleration/de-acceleration upper and lower limits.
